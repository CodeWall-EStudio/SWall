...
g||(b.setSeconds(0),b.setMilliseconds(0));return b.toISOString().split(".")[0]},datetimePickerValueToTs:function(b){if(b){var g=3>b.split(":").length?":00.000Z":".000Z",g=Date.parse(b+g)+6E4*a.time.timezoneOffset;console.log(b,g,new Date(g));return g}return 0}}};return a}]);angular.module("ts.services.user",["ts.utils.constants","ts.services.utils"]).service("UserService",["$rootScope","$http","UtilsService","BACKEND_SERVER","EVENT_LOGIN",function(a,b,g,t,n){return{uid:function(){return g.cookie.get("uid")},hasLoggedIn:function(){var a=g.cookie.get("uid"),b=g.cookie.get("skey");return Boolean(a&&b)},nick:function(){var a=localStorage.login_result;return(a=a?JSON.parse(a):{})?a.nick:""},login:function(g,r,m,q){function e(b,f){console.log("[UserService] login result:",
b,f,document.cookie);200==f&&m?(localStorage.login_result=JSON.stringify(b),m(b,f)):q&&q(b,f);a.$emit(n,f,b)}b.put(t+"/users/"+g+"/login","pwd="+encodeURIComponent(r),{responseType:"json",headers:{"Content-Type":"application/x-www-form-urlencoded"}}).success(e).error(e)},logout:function(){var a=(new Date).toGMTString();document.cookie="uin=; path=/; expires="+a;document.cookie="skey=; path=/; expires="+a;localStorage.removeItem("login_result")},fetchProfile:function(a){}}}]);angular.module("ts.services.activity",["ts.utils.constants","ts.services.utils","ts.services.user"]).service("ActivityService",["$rootScope","$http","UtilsService","UserService","BACKEND_SERVER",function(a,b,g,t,n){function p(c){c?(a.selectedActivity=c,a.selectedActivityID=c._id):(a.selectedActivity=null,a.selectedActivityID="");console.log("[ActivityService] selected activity",a.selectedActivity)}function r(c,h,d,s){e();h=h||{};h._=(new Date).getTime();h=g.object.toUrlencodedString(h);b.put(n+"/activities/"+
c,h,{responseType:"json",headers:{"Content-Type":"application/x-www-form-urlencoded"}}).success(function(c,b){if(c&&!c.c&&c.r)a:{var h=c.r;a.activityMap[h._id]=h;for(var s=0;s<a.activityList.length;++s)for(var f=a.activityList[s].activities,e=0;e<f.length;++e)if(f[e]._id==h._id){a.selectedActivity==f[e]&&(a.selectedActivity=h,a.selectedActivityID=h._id);f[e]=h;break a}}k();d&&d(c,b)}).error(function(a,c){k();s&&s(a,c)})}function m(c,b){a.activityList.splice(c,0,{date:q(b),activities:[b]})}function q(a){a=
...
b.get(n+"/activities/config?_="+d,null,{responseType:"json"}).success(function(b,h){console.log("[ActivityService] activity config =",b);a&&a(b,h)}).error(function(a,c){console.error("[ActivityService] FAIL TO FETCH ACTIVITY CONFIGURATIONS");h&&h(a,c)})}}}]);angular.module("ap.controllers.main",["ts.services.activity","ts.services.user","ts.services.utils"]).controller("PlayerMainController",["$rootScope","$scope","$location","ActivityService","UserService","UtilsService",function(a,b,g,t,n,p){function r(a,b,d,e){a="/activities/"+k+"/videos/"+a;var f=new XMLHttpRequest,g=new FormData;_.each(b,function(a,c){g.append(c,a)});console.log("updating main videos",a,b);f.addEventListener("load",function(a){d&&d(f,a,JSON.parse(f.responseText))});f.addEventListener("error",
function(a){e&&e(f,a)});f.open("PUT",a);f.send(g)}function m(){t.getActivity(k,function(c,h){console.log("[PlayerMainController] get activity success",h,c);if(200==h&&!c.c)if(a.profiles=c.profiles,a.activity=c.r,q(a.activity.resources),a.mainVideos.length||(a.mainVideos=c.r.videos||[]),a.activity.active)a.userCount=a.activity.users.participators.length,b.autoRefresh&&(f=setTimeout(function(){m()},5E3));else{var d=_.countBy(a.activity.resources,function(a){return a.user});a.userCount=_.keys(d).length}},

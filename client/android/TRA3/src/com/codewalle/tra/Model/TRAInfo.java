...
import com.codewalle.tra.utils.DateUtil;
...
*/
    ...
    public TRAInfo(JSONObject activitiesObject){
        init(activitiesObject);
    }

    public TRAInfo(JsonObject activityInfo, JsonObject profiles) throws JSONException {
        // TODO 暂时兼容之前使用 JSONObject 的方式，后面换回 gson
        JSONObject infoObj = new JSONObject(activityInfo.toString());
        JSONObject profileObj = new JSONObject(profiles.toString());
        infoObj.put("profiles",profileObj);
        init(infoObj);
    }

    private void init(JSONObject activitiesObject){
